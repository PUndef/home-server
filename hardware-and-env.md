...
| --------------- | ------------------------------------------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `phoneserver`   | Xiaomi Redmi Note 9 Pro Global (`joyeuse`)  | postmarketOS Edge, no UI, OpenRC | Второй физический узел (8× aarch64, 6 ГБ RAM, 103 GiB root, 8.2 GiB zram). Самозагрузка из Android `boot` partition с кастомным boot.img. Подробно: [phoneserver-pmos-setup.md](phoneserver-pmos-setup.md), скрипты — `[scripts/phoneserver/](scripts/phoneserver/)`. **Пока без постоянного IP в `srv`**: связь только по USB-CDC из WSL Ubuntu на основном ПК (`172.16.42.1`). Wi-Fi на этом ядре пока не поднялся (`ath10k_snoc` / WCN3990), интегрировать в `srv` после решения вопроса с сетью.                                  |
...
- **phoneserver** (Redmi Note 9 Pro / `joyeuse`) — отдельный физический узел на postmarketOS. **Не входит в Proxmox-кластер** (Proxmox VE — x86_64 only), интегрируется как обычный сетевой клиент. Сейчас живёт за пределами `srv`-сегмента: USB-сеть с WSL на основном ПК. Когда появится Wi-Fi/Ethernet — добавить DHCP-резервацию на X3000T в `srv` (по образцу `nextcloud-vm` / `haos17.0`). Подробно: `[phoneserver-pmos-setup.md](phoneserver-pmos-setup.md)`, скрипты — `[scripts/phoneserver/](scripts/phoneserver/)`.
