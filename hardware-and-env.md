...
| Основной ПК пользователя | `pundef-pc`, MAC `9C:6B:00:8B:3F:18` → DHCP `192.168.1.133`; Windows 11 + WSL2 (`networkingMode=mirrored`, тот же IP). zapret bypass + pbr `workvpn` для corp. Cursor Remote SSH с Mac — см. [zapret-bypass-pundef-pc-2026-05-27.md](zapret-bypass-pundef-pc-2026-05-27.md). |
| Рабочий MacBook          | `paul-mac`, MAC `26:C5:4C:20:C5:AD` → DHCP `192.168.1.198`; pbr `workvpn` для corp.                                                                 |
| Xiaomi 13T Pro (тест)    | `xiaomi-13t-pro`, MAC `36:63:0f:4d:4b:5c` → DHCP `192.168.1.204`; corp pbr-policy **снята** (откат stage). Включить снова: `scripts/openwrt/enable-workvpn-client.sh`. |
