param(
    ...
    [string]$User = "deploy",
    [string]$RemotePath = "/srv/static-sites/wf-twitch",
    [string]$KeyPath = "$env:USERPROFILE\.ssh\proxmox_pundef_nopass",
    [string]$Url = "http://wftwitch.home",
    [switch]$SkipBuild
)
...
if (-not $SkipBuild) {
    Write-Host "Building wf-twitch..."
    Push-Location $projectRoot
    try {
        Invoke-Native npm ci
        Invoke-Native npm run sync-events
...
$tmpArchive = Join-Path $env:TEMP ("wf-twitch-dist-{0}.tgz" -f ([guid]::NewGuid().ToString("N")))
