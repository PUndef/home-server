param(
    ...
    [string]$User = "deploy",
    [string]$RemotePath = "/srv/static-sites/warframe",
    [string]$KeyPath = "$env:USERPROFILE\.ssh\proxmox_pundef_nopass",
    [string]$Url = "http://warframe.home",
    [switch]$SkipBuild
)
...
if (-not $SkipBuild) {
    Write-Host "Building warframe-home..."
...
$tmpArchive = Join-Path $env:TEMP ("warframe-dist-{0}.tgz" -f ([guid]::NewGuid().ToString("N")))
