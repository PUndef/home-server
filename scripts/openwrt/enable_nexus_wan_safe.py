"""Safely enable Nexus Mods via WAN for pundef-pc (router-resilience).

Usage:
  py -3 scripts/openwrt/enable_nexus_wan_safe.py
  py -3 scripts/openwrt/enable_nexus_wan_safe.py --dry-run
"""
...
ENABLE_SCRIPT = Path(__file__).resolve().parent / "enable-nexus-wan.sh"
ROLLBACK_SCRIPT = Path(__file__).resolve().parent / "rollback-nexus-wan.sh"
...
def verify_nexus_routing(client: paramiko.SSHClient) -> list[str]:
    failures: list[str] = []
    checks = [
        (
            "ai-tools-policy-nft",
        ...
        (
            "nexus-policy-nft",
            "nft list chain inet fw4 pbr_prerouting | grep -q 'pundef-pc nexus via wan'",
        ),
        (
            "nexus-before-games",
            r"nft list chain inet fw4 pbr_prerouting | awk "
            r"'/pundef-pc nexus via wan/{n=NR} /pundef-pc games via/{g=NR} END{exit !(n && g && n<g)}'",
        ),
        (
            "nexus-resolve",
            "nslookup www.nexusmods.com 192.168.1.1 | grep -qE 'Address.*: (8\\.|104\\.|172\\.)'",
        ),
        (
            "nexus-wan-mark-route",
            "ip route get 8.6.112.6 mark 0x10000 2>/dev/null | grep -q ' dev wan '",
        ),
        (
            "corp-workvpn-mark",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Safely enable Nexus Mods via WAN on pundef-pc")
    ...
    try:
        ...
        route_fail = verify_nexus_routing(client)
        ...
        print("\n=== Success ===")
        print("Nexus Mods (*.nexusmods.com) from pundef-pc (.133) -> WAN.")
        print("Other egress on this PC still -> awg2 (games catch-all).")
        print("Test: open nexusmods.com, try a mod download.")
        return 0
    finally:
        client.close()
