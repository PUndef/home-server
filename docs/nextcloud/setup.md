...
| 2026-03-15 | **Скрипт бэкапа + cron** — `/usr/local/bin/nextcloud-backup.sh` (tar.gz + gzip), воскресенье 3:00, лог `/var/log/nextcloud-backup.log`. |
| 2026-05-28 | **Ротация бэкапов → один набор** — скрипт в репо [`nextcloud-vm/nextcloud-backup.sh`](../../nextcloud-vm/nextcloud-backup.sh); перед запуском удаляются старые архивы; свежий бэкап `20260528_134038`. |
...
**Экономия места:** каталоги в **tar.gz**, дамп БД в **.bak.gz**. Бэкап по расписанию **раз в неделю** (воскресенье 3:00), хранить **один набор** (перед запуском скрипт удаляет предыдущие архивы — иначе на диске ~100 ГБ не хватит места под второй `data_*.tar.gz`).
...
**Сделано:** ручной бэкап (Шаг 0.1), скрипт со сжатием + cron по воскресеньям 3:00, ротация **один набор**. См. [Сделано (история)](#сделано-история).
